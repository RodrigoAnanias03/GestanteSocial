 ...
 * @author JONATAS
 */
public class EditarAmizadeFamiliarDAO extends Conexao{
    public EditarAmizadeFamiliarDAO()throws Exception{}
    ...
    public boolean adicionarAmigo(int id, int idGestante){
        int x=0;
        try {
            String sql = "insert into amigos_familiar(status_amizade, autor_amizade, id_gestante, id_familiar) "
                    + "values(false, 'familiar', ?, ?)";
            PreparedStatement ps = getConexao().prepareStatement(sql);
    ...
    public void aceitarSolicitacao(int id, int idGestante){
        try {
            String sql = "update amigos_familiar set status_amizade = true where id_familiar=? and id_gestante=?";
            PreparedStatement ps = getConexao().prepareStatement(sql);
            ps.setInt(1, id);
    ...
    public void excluirSolicitacao (int id, int idGestante){
        try {
            String sql="delete from amigos_familiar where id_familiar=? and id_gestante=?";
            PreparedStatement ps = getConexao().prepareStatement(sql);
            ps.setInt(1, id);
            ps.setInt(2, idGestante);
