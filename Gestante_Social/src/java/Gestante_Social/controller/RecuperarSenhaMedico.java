 ...
 * @author JONATAS
 */
@WebServlet(name = "RecuperarSenhaMedico", urlPatterns = {"/RecuperarSenhaMedico"})
public class RecuperarSenhaMedico extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (dao.verificaEmailMedico(email)) {
                dao = new RecuperarSenhaDAO();
                if (dao.verificaAtivoMedico(email)) {
                    dao = new RecuperarSenhaDAO();
                    Medico med = dao.buscarDadosMedico(email);
                    Email_Senha emailSenha = new Email_Senha();
                    emailSenha.enviar(email, med.getUsuario(), med.getSenha());

                    request.setAttribute("email", email);
                    request.getRequestDispatcher("recuperacao_finalizar.jsp").forward(request, response);

                } else {
                    request.setAttribute("mensagem", "Usuário Inativo! Para ativar entrar em contato!");
                }
            ...
            request.getRequestDispatcher("recup_senha_medico.jsp").forward(request, response);
