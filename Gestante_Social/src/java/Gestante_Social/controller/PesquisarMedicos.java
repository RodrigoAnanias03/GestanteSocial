 ...
 * @author JONATAS
 */
@WebServlet(name = "PesquisarMedicos", urlPatterns = {"/PesquisarMedicos"})
public class PesquisarMedicos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            nomeMedico = Normalizer.normalize(nomeMedico, Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]", "");
            nomeMedico = nomeMedico.toUpperCase();
            }else{
                nomeMedico = (String) request.getAttribute("nomePesquisa");
            }
            
            BuscarMedicoDAO dao;
            dao = new BuscarMedicoDAO();
            
            ArrayList<Medico> medico = dao.pesquisarMedico(nomeMedico, obj.getIdUsuario());
            
            request.setAttribute("medico", medico);
            request.setAttribute("nomePesquisa", nomeMedico);
            request.setAttribute("quantidade", medico.size());
            request.getRequestDispatcher("resultado_pesquisa_medico.jsp").forward(request, response);
        } catch (Exception ex) {
