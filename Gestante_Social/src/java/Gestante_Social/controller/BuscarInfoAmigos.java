 ...
 * @author JONATAS
 */
@WebServlet(name = "BuscarInfoAmigos", urlPatterns = {"/BuscarInfoAmigos"})
public class BuscarInfoAmigos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String loginAmigo = String.valueOf(request.getParameter("idAmigo"));

            if (loginAmigo.equals("null")) {
                loginAmigo = String.valueOf(request.getAttribute("idAmigo"));
            }

            if (loginAmigo.equals("null")) {
                loginAmigo = Cripto(obj.getLogin());
            }
            ...
            if (amigo.getIdUsuario() == obj.getIdUsuario()) {
                request.setAttribute("situacao_amigo", "Você");
                request.setAttribute("botao", "Você");
            } else {
                dao = new BuscarInfoAmigosDAO();
                int x = dao.buscarStatusAmizade(obj.getIdUsuario(), amigo.getIdUsuario());
                if (x == 0) {
            ...
            request.getRequestDispatcher("perfil_amigo.jsp").forward(request, response);
        } catch (Exception ex) {
            request.setAttribute("erro", String.valueOf(ex));
            request.getRequestDispatcher("tela_erro.jsp").forward(request, response);
            System.out.println("Erro em BuscarInfoAmigos.processRequest()! Erro!" + ex.getMessage());
            ex.printStackTrace();
        }
    }
