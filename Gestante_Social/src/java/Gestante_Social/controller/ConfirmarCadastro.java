 ...
 * @author JONATAS
 */
@WebServlet(name = "ConfirmarCadastro", urlPatterns = {"/ConfirmarCadastro", "/confirmarcadastro"})
public class ConfirmarCadastro extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        UsuarioDAO dao;
        try {
            String id = Decripto(request.getParameter("idUsuario"));

            dao = new UsuarioDAO();
            if (dao.verificaAtivo(Integer.parseInt(id))) {

                dao = new UsuarioDAO();
                dao.ativaUsuario(Integer.parseInt(id));

                dao = new UsuarioDAO();
                String nome = dao.pegaNome(Integer.parseInt(id));
                request.setAttribute("nomeUsuario", nome);
                request.getRequestDispatcher("confirmar_cadastro.jsp").forward(request, response);
            }else{
            ...
            System.out.println("Erro no ConfirmarCadastro! Erro " + ex.getMessage());
            ex.printStackTrace();
        }
    }
