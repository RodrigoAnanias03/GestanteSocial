...
import Gestante_Social.dao.ReenviarEmailDAO;
 ...
 * @author JONATAS
 */
@WebServlet(name = "ReenviarEmail", urlPatterns = {"/ReenviarEmail"})
public class ReenviarEmail extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ReenviarEmailDAO dao;
            String email = request.getParameter("emailUsuario");
            dao = new ReenviarEmailDAO();
            if (!dao.verEmail(email)) {
                request.setAttribute("info", "Email não cadastrado");
                request.getRequestDispatcher("reenviar_email.jsp").forward(request, response);
            } else {
                String url = String.valueOf(request.getRequestURL());
                dao = new ReenviarEmailDAO();
                url = url.replaceAll("ReenviarEmail", "confirmarcadastro?idUsuario=" + Cripto(String.valueOf(dao.pegarId(email))));
                Enviar_Email env_email = new Enviar_Email();
                env_email.reEnviar(email, url);
